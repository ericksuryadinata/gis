...
$(document).ready(function () {
	...
	$(".show-modal-tambah").on('click',function () {
		...
		google.maps.event.addListener(mapTambah,'click', function(event){
			...
			if(isPusatKotaClicked == true){
				$("[name='P9-pusat-kota']").parent().addClass('is-dirty');
				$("[name='P9-pusat-kota']").text(locationClicked.lat().toFixed(4) + " " + locationClicked.lng().toFixed(4));
				$("[name='P9-pusat-kota']").val(locationClicked.lat().toFixed(4) + " " + locationClicked.lng().toFixed(4));
				isPusatKotaClicked = false;
			}

			if(isPusatUkmClicked == true){
				$("[name='P9-pusat-ukm']").parent().addClass('is-dirty');
				$("[name='P9-pusat-ukm']").val(locationClicked.lat().toFixed(4) + " " + locationClicked.lng().toFixed(4));
				isPusatUkmClicked = false;
			}

			if(isWilayahClicked == true){
				if($("[name='P9-wilayah']").val() != ''){
					$("[name='P9-wilayah']").parent().addClass('is-dirty');
					$("[name='P9-wilayah']").val($("[name='P9-wilayah']").val()+","+locationClicked.lat().toFixed(4) + " " + locationClicked.lng().toFixed(4));
				}else{
					$("[name='P9-wilayah']").parent().addClass('is-dirty');
					$("[name='P9-wilayah']").val(locationClicked.lat().toFixed(4) + " " + locationClicked.lng().toFixed(4));
				}
			}
		})
	...
	$("#table-body-data").on('click','.show-modal-edit', function () {
		...
		$.ajax({
			...
			success: function (response) {
				...
				lokasiWilayah = lokasiWilayah.replace(/\)/g,"");
				$("[name='P9-id-edit']").val(response.id);
				$("[name='P9-kode-kabupaten-edit']").val(response.kode_kabupaten);
				$("[name='P9-kode-kabupaten-edit']").parent().addClass('is-dirty');
				$("[name='P9-nama-kabupaten-edit']").val(response.nama_kabupaten);
				$("[name='P9-nama-kabupaten-edit']").parent().addClass('is-dirty');
				$("[name='P9-nama-bupati-edit']").val(response.nama_bupati);
				$("[name='P9-nama-bupati-edit']").parent().addClass('is-dirty');
				$("[name='P9-jumlah-penduduk-edit']").val(response.jumlah_penduduk);
				$("[name='P9-jumlah-penduduk-edit']").parent().addClass('is-dirty');
				$("[name='P9-jumlah-ukm-edit']").val(response.jumlah_ukm);
				$("[name='P9-jumlah-ukm-edit']").parent().addClass('is-dirty');
				$("[name='P9-pusat-kota-edit']").val(lokasiPusatKota);
				$("[name='P9-pusat-kota-edit']").parent().addClass('is-dirty');
				$("[name='P9-pusat-ukm-edit']").val(lokasiPusatUkm);
				$("[name='P9-pusat-ukm-edit']").parent().addClass('is-dirty');
				$("[name='P9-wilayah-edit']").val(lokasiWilayah);
				$("[name='P9-wilayah-edit']").parent().addClass('is-dirty');
				lokasiPusatKota = lokasiPusatKota.split(' ');
				...
				google.maps.event.addListener(mapEdit,'click', function(event){
					...
					if(isPusatKotaEditClicked == true){
						$("[name='P9-pusat-kota-edit']").parent().addClass('is-dirty');
						$("[name='P9-pusat-kota-edit']").text(locationEditClicked.lat().toFixed(4) + " " + locationEditClicked.lng().toFixed(4));
						$("[name='P9-pusat-kota-edit']").val(locationEditClicked.lat().toFixed(4) + " " + locationEditClicked.lng().toFixed(4));
						isPusatKotaEditClicked = false;
					}

					if(isPusatUkmEditClicked == true){
						$("[name='P9-pusat-ukm-edit']").parent().addClass('is-dirty');
						$("[name='P9-pusat-ukm-edit']").val(locationEditClicked.lat().toFixed(4) + " " + locationEditClicked.lng().toFixed(4));
						isPusatUkmEditClicked = false;
					}

					if(isWilayahEditClicked == true){
						if($("[name='P9-wilayah-edit']").val() != ''){
							if(isEditMode == false){
								$("[name='P9-wilayah-edit']").parent().addClass('is-dirty');
								$("[name='P9-wilayah-edit']").val('');	
								isEditMode = true;
								$("[name='P9-wilayah-edit']").val(locationEditClicked.lat().toFixed(4) + " " + locationEditClicked.lng().toFixed(4));
							}else{
								$("[name='P9-wilayah-edit']").parent().addClass('is-dirty');
								$("[name='P9-wilayah-edit']").val($("[name='P9-wilayah-edit']").val()+","+locationEditClicked.lat().toFixed(4) + " " + locationEditClicked.lng().toFixed(4));
							}
						}else{
							$("[name='P9-wilayah-edit']").parent().addClass('is-dirty');
							$("[name='P9-wilayah-edit']").val(locationEditClicked.lat().toFixed(4) + " " + locationEditClicked.lng().toFixed(4));
	...
	$("#simpan-edit-data").on('click', function () {
        $.ajax({
            type: "POST",
            url: base_url+'lihat/updateMapP9',
	...
	$("#table-body-data").on('click','.delete-data', function () {
        ...
        $.ajax({
            type: "POST",
            url: base_url+'lihat/deleteMapP9',
            data: {'id':id},
            dataType: "JSON",
            success: function (response) {
                if(response.status == true){
	...
	$("#cari-data").on('click', function () {
		$.ajax({
			type: "POST",
			url: base_url+'lihat/searchMapP9',
